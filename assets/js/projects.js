...
function wakeCard(cardEl, on) {
  ...
  (cardModule || (cardModule = import('./cards.js?v=8f3d805d')))
    .then(({ initCardModel }) => {
...
function startSim(slug, d) {
  ...
  const load = kind === 'fault'
    ? import('./rig.js?v=8f3d805d').then(({ initRig }) => initRig(canvas, { reducedMotion: reduced }))
    : import('./cards.js?v=8f3d805d').then(({ initCardModel }) => initCardModel(canvas, 'scan'));
